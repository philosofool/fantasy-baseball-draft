...
def main():
    ##Step one: project each position and write positional projections to a file
    ...
    ##Step two: write the positional projections to a master "all hitters" file.
    ...
    for sheet in sheets:
        ...
        for line in file.readlines():
            master.write(positions[n] + ",")#write the players position
            master.write(line)#write his projection
        file.close()
        n = n + 1
    master.close()
    ##Step 3... having a hard time seeing the value, but it makes a list of "new hitters"
    ...
    for line in redundant.readlines():
        #print(line)
        if line.split(",")[1] in master_set: #looks to see if a player is in master set...
            pass
        else:
            #print(line)
...
def project(infile,outfile):
    ZiPS = leaders("SteamerHitters_2016.csv")
    fans = leaders(infile)
    aggregate = []
